<?php
namespace Opencart\Admin\Model\Extension\Tmdcustomheadermenu\Tmd;
class Tmdcustomheader extends \Opencart\System\Engine\Model {
...
$this->db->query("CREATE TABLE IF NOT EXISTS `".DB_PREFIX."headermenu_description` (
  ...
  `link` text NOT NULL
	...
	public function addheadermenu($data) {
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "headermenu SET  level1 = '" .$data['level1'] . "', level2 = '" .$data['level2'] . "', `column` = '" .$data['column'] . "',status = '" . (int)$data['status'] . "', sort_order = '" . (int)$data['sort_order'] . "'");

		$headermenu_id = $this->db->getLastId(); 
		
		foreach ($data['headermenu_description'] as $language_id => $value) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "headermenu_description SET `headermenu_id` = '" . (int)$headermenu_id . "', language_id = '" . (int)$language_id . "', title = '" . $this->db->escape((string)$value['title']) . "', link = '" . $this->db->escape((string)$value['link']) . "'");
		}
	...
	public function editheadermenu($headermenu_id, $data) {
		...
		$this->db->query("UPDATE " . DB_PREFIX . "headermenu SET  level1 = '" .$data['level1'] . "' , level2 = '" .$data['level2'] . "', `column` = '" .$data['column'] . "',status = '" . (int)$data['status'] . "', sort_order = '" . (int)$data['sort_order'] . "' WHERE headermenu_id = '" . (int)$headermenu_id . "'");
	...
	public function getheadermenus($data = []) {
		if ($data) {
	...
	public function getheadermenus1($data = []) {
